...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleGuestLogin = async () => {
    try {
      const guestData = {
        email: 'guest@pomorix.space',
        auth_provider: AuthProvider.GOOGLE,
        auth_provider_id: 'guest-mode',
      };

      const success = await socialLogin(guestData);
      ...
      <main className="relative z-10 w-full max-w-[400px] bg-white shadow-sharp-lg border border-slate-200 flex flex-col p-10">
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <button
            ...
            className="flex items-center justify-center gap-3 bg-slate-50 hover:bg-slate-100 border border-slate-300 text-slate-700 py-4 transition-all duration-200 text-base font-bold shadow-sharp group w-full active:translate-y-[1px]"
            type="button"
            disabled={loading}
          >
            {/* Person Icon */}
            <span className="material-symbols-outlined text-slate-600 group-hover:scale-110 transition-transform">person</span>
